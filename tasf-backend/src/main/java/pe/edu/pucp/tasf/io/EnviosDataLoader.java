...
public class EnviosDataLoader {
    ...
    /** Mapa opcional ICAO -> offset GMT (horas) para convertir hora local a UTC. */
    private Map<String, Integer> gmtByCode = Collections.emptyMap();

    /** Configura el mapa de husos horarios a usar durante el parseo. */
    public void setGmtMap(Map<String, Integer> gmtMap) {
        this.gmtByCode = (gmtMap == null) ? Collections.emptyMap() : gmtMap;
    }
    ...
    private ShipmentRequest parseLine(String line, String originIcao,
        ...
        try {
            ...
            double fracDayLocal = (hour + minute / 60.0) / 24.0;
            int gmtOrig = gmtByCode.getOrDefault(originIcao, 0);
            double creationTime = daysOffset + fracDayLocal - gmtOrig / 24.0;
