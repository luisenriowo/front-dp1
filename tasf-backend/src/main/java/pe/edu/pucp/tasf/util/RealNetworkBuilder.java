...
public class RealNetworkBuilder {
    ...
    public LogisticsNetwork build(List<Airport> airports, Path flightsFile) throws IOException {
        return build(airports, flightsFile, Collections.emptyMap());
    }

    /**
     * Construye la red logística usando además husos horarios por ICAO para
     * convertir los horarios locales de planes_vuelo.txt a un eje UTC común.
     */
    public LogisticsNetwork build(List<Airport> airports, Path flightsFile, Map<String, Integer> gmtMap) throws IOException {
        LogisticsNetwork network = new LogisticsNetwork();
        for (Airport a : airports) {
            network.addAirport(a);
        }
        RealFlightLoader loader = new RealFlightLoader();
        int loaded = loader.load(flightsFile, network, gmtMap);
